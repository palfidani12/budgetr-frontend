...
function App() {
  ...
  useEffect(() => {
    ...
    if (savedBudgets) {
      ...
      setBudgetGoals(
        parsed.map((b: any) => ({ ...b, startDate: new Date(b.startDate) }))
      );
    }
  }, []);
      ...
      switch (r.frequency) {
        ...
        case "weekly": {
          const diff = Math.floor(
            (new Date().getTime() - r.startDate.getTime()) /
              (1000 * 60 * 60 * 24 * 7)
          );
 ...
 */
  ...
  const removeAttachment = (attachmentId: string) => {
    setNewTransaction({
      ...newTransaction,
      attachments: newTransaction.attachments.filter(
        (a) => a.id !== attachmentId
      ),
    });
  };
    ...
    if (newTransaction.isRecurring) {
      const rec: RecurringTransaction = {
        ...
        endDate: newTransaction.endDate
          ? new Date(newTransaction.endDate)
          : undefined,
  ...
  const totalIncome = transactions
    ...
    .reduce(
      (sum, t) => sum + convertCurrency(t.amount, t.currency, selectedCurrency),
      0
    );
  ...
  const getBudgetProgress = (
    ...
    const spent = transactions
      .filter(
        (t) =>
          t.category === category && t.type === "expense" && t.date >= start
      )
      .reduce(
        (sum, t) =>
          sum + convertCurrency(t.amount, t.currency, selectedCurrency),
  ...
  const renderWidget = (widget: Widget) => {
    ...
    switch (widget.type) {
      case "top-categories":
        ...
        return (
          <div key={widget.id} className="widget top-categories-widget">
      ...
      case "largest-expenses":
        ...
        return (
          <div key={widget.id} className="widget largest-expenses-widget">
            <div className="widget-header">
              <h3>{widget.title}</h3>
              <button
                className="widget-toggle"
      ...
      case "savings-rate":
        ...
        return (
          <div key={widget.id} className="widget savings-rate-widget">
            <div className="widget-header">
              <h3>{widget.title}</h3>
              <button
                className="widget-toggle"
            ...
            <div className="widget-content">
              <div className="savings-rate-display">
                <div className="savings-rate-circle">
                  <div className="savings-rate-value">
      ...
      case "budget-alerts":
        ...
        return (
          <div key={widget.id} className="widget budget-alerts-widget">
            <div className="widget-header">
              <h3>{widget.title}</h3>
              <button
                className="widget-toggle"
            ...
            <div className="widget-content">
              {budgetAlerts.map((alert, index) => (
                <div
                  ...
                  className={`alert-item ${
                    alert.percentage > 100 ? "over" : "warning"
                  }`}
                >
              ...
              {budgetAlerts.length === 0 && (
                <p className="no-data">All budgets are on track!</p>
              )}
            </div>
          </div>
        );
                    ...
                    return (
                      ...
                      >
                        ...
                        <button
                          ...
                          onClick={() =>
                            setBudgetGoals(
                              budgetGoals.filter((g) => g.id !== goal.id)
                            )
                          }
                        >
                          ×
            ...
            <div className="transactions-list">
              {recurring.map((r) => (
                <div key={r.id} className="transaction-item">
                  <div className="transaction-info">
                    <div className="transaction-category">
                      {categories.find((c) => c.name === r.category)?.icon ||
                        "🔁"}
                    </div>
                    <div className="transaction-details">
                      <h4>{r.description}</h4>
                      <p>
                        {r.category} •{" "}
                        {r.frequency.charAt(0).toUpperCase() +
                          r.frequency.slice(1)}{" "}
                        • Start: {format(r.startDate, "MMM dd, yyyy")}
                        {r.endDate
                          ? ` • End: ${format(r.endDate, "MMM dd, yyyy")}`
                          : ""}
                      </p>
                    </div>
                  </div>
        ...
        <div className="modal-overlay" onClick={() => setShowAddModal(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-actions">
              ...
              <button className="save-button" onClick={addTransaction}>
                Save Transaction
              </button>
            </div>
          </div>
          ...
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="form-group">
              ...
              <input
                ...
                onChange={(e) =>
                  setNewBudgetGoal({ ...newBudgetGoal, amount: e.target.value })
                }
              />
            </div>
            ...
            <div className="modal-actions">
              ...
              <button className="save-button" onClick={addBudgetGoal}>
                Save Budget Goal
              </button>
            </div>
          </div>
